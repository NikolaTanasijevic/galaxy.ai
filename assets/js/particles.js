...
const gmDefaultColors = ['#ffffff', '#ffffff', '#ffffff']
const gmHexToRgb = hex => {
...
const gmNum = (v, fallback) => {
	const n = Number(v)
	return Number.isFinite(n) ? n : fallback
}
const gmBool = (v, fallback) => {
...
const gmMat4Perspective = (out, fovy, aspect, near, far) => {
...
`
function gmInitParticles(container) {
	const particleCount = Math.max(1, Math.floor(gmNum(container.dataset.count, 200)))
	const particleSpread = gmNum(container.dataset.spread, 10)
	const speed = gmNum(container.dataset.speed, 0.1)
	const particleColors = (container.dataset.colors || '')
		.split(',')
		.map(s => s.trim())
		.filter(Boolean)
	const moveParticlesOnHover = gmBool(container.dataset.hover, false)
	const particleHoverFactor = gmNum(container.dataset.hoverFactor, 1)
	const alphaParticles = gmBool(container.dataset.alpha, false)
	const particleBaseSize = gmNum(container.dataset.baseSize, 100)
	const sizeRandomness = gmNum(container.dataset.sizeRandomness, 1)
	const cameraDistance = gmNum(container.dataset.cameraDistance, 20)
	const disableRotation = !gmBool(container.dataset.rotation, true)
	const pixelRatio = gmNum(container.dataset.dpr, Math.min(window.devicePixelRatio || 1, 2))
	...
	const vs = compile(gl.VERTEX_SHADER, gmParticleVS)
	const fs = compile(gl.FRAGMENT_SHADER, gmParticleFS)
	if (!vs || !fs) return () => {}
	...
	const palette = particleColors.length > 0 ? particleColors : gmDefaultColors
	...
	for (let i = 0; i < count; i++) {
		...
		const col = gmHexToRgb(palette[Math.floor(Math.random() * palette.length)])
		colors.set(col, i * 3)
	}
	...
	// listens on window rather than the container, since the container has
	// pointer-events:none so it doesn't intercept clicks on hero content sitting above it
	...
	const resize = () => {
		...
		canvas.height = h
		gl.viewport(0, 0, w, h)
		gmMat4Perspective(proj, (15 * Math.PI) / 180, w / h, 0.1, 1e3)
		gl.uniformMatrix4fv(uProjLoc, false, proj)
	}
	window.addEventListener('resize', resize, false)
	resize()

	let posX = 0, posY = 0, rotX = 0, rotY = 0, rotZ = 0
	...
	const frame = t => {
		...
		gmMat4Identity(model)
		gmMat4Translate(model, model, [posX, posY, 0])
		gmMat4RotateX(model, model, rotX)
		gmMat4RotateY(model, model, rotY)
		gmMat4RotateZ(model, model, rotZ)
		gl.uniformMatrix4fv(uModelLoc, false, model)
		gmMat4Identity(view)
		gmMat4Translate(view, view, [0, 0, -cameraDistance])
		gl.uniformMatrix4fv(uViewLoc, false, view)
		gl.drawArrays(gl.POINTS, 0, count)
		if (isRunning) animationFrameId = requestAnimationFrame(frame)
	}
	const start = () => {
		if (isRunning) return
		isRunning = true
	...
	// pause rendering while the hero is scrolled out of view to save GPU/battery
